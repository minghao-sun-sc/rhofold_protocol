...
class RNADataset(Dataset):
    ...
    def __build_core_data_cplx(self, rna_complex_id):
        ...
        for i, (s, c, cmsk, m_t, r_t) in enumerate(zip(aa_seq_list,
            ...
            c = c.reshape([len(s), self.n_grps_cord, 3])
            cmsk = cmsk.reshape([len(s), self.n_grps_cord])
...
def unitest_complex():
    ...
    data_dir = '/public/home/taoshen/data/rna/RNA3D/data_px'
    id_fpath = '/public/home/taoshen/data/rna/RNA3D/data_px/exp/all_seq/train_complex.txt'
    # id_fpath = '/public/home/taoshen/data/rna/RNA3D/data_px/exp/all_seq/valid_complex_rnapuzzle.txt'

    clstr_list = None
    clstr_id_list = None

    fas_dpath = f'{data_dir}/key_seq_v2/'           # directory path to SEQ files
    msa_dpath = f'{data_dir}/msa/'
    pdb_dpath = f'{data_dir}/full_pdb/'          # directory path to KEYPDB files
    pdb_version = 'v0.0'
    frame_version = 'v2'
    ...
    ds_train_dataset = RNADataset(ds_train_config, is_train=False)
...
def unitest_clstr():
    ...
    id_fpath = None
    clstr_list = '/public/home/taoshen/data/rna/RNA3D/data_px/cdhit_out_0.8.clstr'
    clstr_id_list = '/public/home/taoshen/data/rna/RNA3D/data_px/cdhit_out_0.8_trn_clstr.list'

    fas_dpath = f'{data_dir}/key_seq_v2/'           # directory path to SEQ files
    msa_dpath = f'{data_dir}/msa/'
    pdb_dpath = f'{data_dir}/key_pdb_v2.1/'         # directory path to KEYPDB files
...
def unitest_list():
    ...
    id_fpath = f'/public/home/taoshen/data/rna/RNA3D/Ash_Total_RNA_test/exp/paper1.0/train_ids_no_cplx'

    msa_dpath = f'{data_dir}/rMSA/'
    fas_dpath = f'{data_dir}/rna_seq/'              # directory path to SEQ files
    pdb_dpath = f'{data_dir}/rna_pdb_align_pad/'    # directory path to KEYPDB files
    danpz_dpath = f'{data_dir}/da_npzs_pad_pseudo/'
    dr_ss_dpath = None
    dr_cm_dpath = None
    ss_dpath =  None
    dr_fea_dpath = None
    ...
    # id_fpath = f'{data_dir}/v1.0_n23170'
    # msa_dpath = f'{data_dir}/rMSA/'
    # fas_dpath = f'{data_dir}/seq/'                                                # directory path to SEQ files
    # pdb_dpath = f'{data_dir}/standard_model_1_refined_align/'                     # directory path to KEYPDB files
    # danpz_dpath = f'{data_dir}/standard_model_1_unrefined_align_da_npzs/'
    # ss_dpath = f'{data_dir}/ssmap/'
    # dr_ss_dpath = None
    # dr_cm_dpath = None
    # dr_fea_dpath = None
    '''
    ...
    for i in tqdm(range(len(ds_train_dataset))):
        ...
        #     print(k, data_dict['dr_da_lbls_soft'][k].shape)
        # for key in data_dict.keys():
        #     if key in set(tns_keys) and exists(data_dict[key]):
        #         print(i, key, data_dict[key].shape)
        # exit()
...
def unitest_list_10cv():
    ...
    id_fpath = f'/public/home/taoshen/code/Projects/E2EFold3D_Dev/data_pipeline/stats/nrlist_3.226_4.0A_single_all.txt.cdhit_out0.8.ids'
